...
const authenticate = (loginType: LoginMethod, userName: string, phoneNum: string, password: string, code: string): Promise<AccountInfo> => {
    return new Promise((success, fail) => {
        GetClient(AuthClient).login(LoginRequest.create({
            method: loginType,
            phoneNum: phoneNum,
            username: userName,
            password: password,
            ...
            .response.then((re: LoginReply) => {
                success(new AccountInfo(
                    Number(re.uid),
                    re.username,
                    re.jwt
                ))
 ...
 * @constructor
 */
export const Login = (loginType: LoginMethod, userName: string, phoneNum: string, password: string, code: string, success?: (userinfo: AccountInfo) => void, fail?: (reason?: any) => void) => {
    authenticate(loginType, userName, phoneNum, password, code).then(
        (info) => {
...
export const RenewJWT = ():Promise<void>|void => {
   ...
   return GetClient(AuthClient).renewToken(RenewTokenRequest.create({jwt: authStore().jwt}), {disableUIIndicator: true})
        .response.then((re: RenewTokenReply) => {
