...
<template>
  <div class="-intro-y">
    ...
    <div class="p-5">
      <Alerts variant="warning" icon="AlertTriangle" :with-close-icon="true" :messages="warningAlerts" class="flex mx-4 mb-3 items-center" />

      <Alerts variant="primary" icon="AlertCircle" :with-close-icon="true" :auto-dismiss="5000" :messages="infoAlerts" class="flex mx-4 mb-3 items-center" />
      <div>
        <FormLabel htmlFor="change-password-form-1">
          旧密码
          <span class="text-[red]">*</span>
        </FormLabel>
        <FormInput autocomplete="off" type="password" placeholder="请输入旧密码" v-model="formData.oldPassword" />
        <template v-if="errors.oldPassword.errors">
          <div v-for="(error, index) in errors.oldPassword.errors" :key="index" class="mt-2 text-danger">
            {{ error.message }}
          </div>
        </template>
      </div>
      <div class="mt-3">
        <FormLabel htmlFor="change-password-form-2">
          新密码
          <span class="text-[red]">*</span>
          <div class="text-xs text-left pt-1 sm:ml-auto sm:mt-0 text-slate-500">密码建议大于6位，包含大小写及特殊字符.</div>
        </FormLabel>
        <FormInput autocomplete="off" v-model="formData.newPassword" type="password" placeholder="请输入新密码" />
        <template v-if="errors.newPassword.errors">
          <div v-for="(error, index) in errors.newPassword.errors" :key="index" class="mt-2 text-danger">
            {{ error.message }}
          </div>
        </template>
      </div>
      ...
      <div class="mt-3">
        ...
        <FormInput autocomplete="off" v-model="formData.newPassword2" type="password" placeholder="请输入新密码" />
        <template v-if="errors.newPassword2.errors">
          <div v-for="(error, index) in errors.newPassword2.errors" :key="index" class="mt-2 text-danger">
            {{ error.message }}
          </div>
        </template>
      </div>
      <Button variant="primary" type="button" class="mt-4" @click="savePassword">保存</Button>
      <Button
        variant="secondary"
        type="button"
        class="mt-4 ml-4"
