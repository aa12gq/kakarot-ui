 ...
 */
const toSchemaErrors = <R extends yup.AnyObject, K extends keyof R, E extends { [key in K]: RuleError }>(schema: yup.ObjectSchema<R>, errors: ValidationError[], errorRef?: Ref<any>): Ref<E> => {
 ...
 */
const autoWatch = <R extends yup.AnyObject, K extends keyof R, E extends { [key in K]: RuleError }>(schema: yup.ObjectSchema<R>, formData: any, errorRef: Ref<E>) => {
  watch(formData, () => {
...
yup.setLocale({
  mixed: {
    required: o => `${o.label}为必填项`,
  },
  string: {
    min: o => `${o.label}不能少于${o.min}字符`,
    max: o => `${o.label}不能大于${o.max}字符`,
    // 验证手机号
    matches: o => `${o.label}格式不正确`,
  },
  number: {
    min: o => `${o.label}不能小于${o.min}`,
    max: o => `${o.label}不能大于${o.max}`,
    integer: o => `${o.label}必须是整数`,
  },
  date: {
    min: o => `${o.label}不能小于${o.min}`,
    max: o => `${o.label}不能大于${o.max}`,
